...
function initSteganographyActions() {
    ...
    textEncryptBtn.addEventListener('click', () => {
        ...
        // Always use steganographic output
        ...
        try {
            ...
            // Save operation to history if API client is available
            try {
                if (typeof window.apiClient !== 'undefined' && window.apiClient) {
                    // Save operation to history
                    window.apiClient.saveOperation({
                        type: 'text',
                        mode: 'encrypt',
                        ...
                        console.error('Error saving operation to history:', err);
                        // Don't show error to user, this is a background operation
                    });
                }
            } catch (apiError) {
                console.error('Error saving operation to history:', apiError);
                // Don't show error to user, this is a background operation
            }

            // Show appropriate success message based on password usage
            if (password) {
                showNotification(
    ...
    textDecryptBtn.addEventListener('click', () => {
        ...
        try {
            ...
            if (error.message.includes('Wrong password') ||
                ...
                // Always show the same message for password issues
                showNotification(
                    'Wrong password, couldn\'t decrypt',
                    'error',
                    'The provided password does not match the one used for encryption.'
                );
            } else {
                // For other errors, show the specific error message
                showNotification('Decryption failed', 'error', error.message);
            }
        }
    });
...
function decryptMessage(encrypted, password) {
    try {
        ...
        // Always return the decrypted result, regardless of password correctness
        // This allows users to see the actual encrypted message
        return decrypted;
    } catch (error) {
