...
class ApiClient {
  constructor() {
    // Wait for config to be ready or use defaults
    ...
    window.addEventListener('configReady', this.handleConfigReady.bind(this));
  }
  ...
  initializeConfig() {
    ...
    this.baseUrl = window.CONFIG ? window.CONFIG.apiBaseUrl : this.getDefaultApiUrl();
    this.apiVersion = window.CONFIG ? window.CONFIG.apiVersion : 'v1';
    this.token = localStorage.getItem(window.CONFIG ? window.CONFIG.tokenKey : 'jwt_token');
    this.refreshToken = localStorage.getItem(window.CONFIG ? window.CONFIG.refreshTokenKey : 'refresh_token');
  }

  /**
   * Get default API URL based on current hostname
   */
  getDefaultApiUrl() {
    ...
    if (isLocalhost) {
      ...
      return 'https://lasavot-backend.onrender.com/api';
    } else {
  ...
  handleConfigReady(event) {
    ...
    if (config.debug) {
      console.log('ApiClient configuration updated:', {
