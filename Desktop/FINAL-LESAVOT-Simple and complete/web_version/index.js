...
document.addEventListener('DOMContentLoaded', async function() {
    // Diffie-Hellman Key Exchange integration
    window.sharedSecret = null;
    try {
        const dh = new DiffieHellmanKeyExchange();
        await dh.fetchServerParams();
        await dh.generateClientKeyPair();
        const secret = await dh.completeKeyExchange();
        window.sharedSecret = secret;
        showNotification('Secure session established (Diffie-Hellman)', 'success');
    } catch (e) {
        showNotification('Key exchange failed: ' + e.message, 'error');
    }
